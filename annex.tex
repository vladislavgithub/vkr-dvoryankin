% --- Приложение А ----------------------------------------------------------
\section*{ПРИЛОЖЕНИЕ А\\[2pt]\normalfont\normalsize\mdseries Фрагменты исходного кода}
\label{application_A}
\addcontentsline{toc}{section}{ПРИЛОЖЕНИЕ А. Фрагменты исходного кода}
\pagestyle{empty}

% Перенумерация листингов внутри приложения А: А.1, А.2, ...
\renewcommand{\thelstlisting}{А.\arabic{lstlisting}}
\setcounter{lstlisting}{0}
% Перенумерация рисунков и таблиц по букве приложения.
...
\begin{lstlisting}[caption={Формирование q-score из (корректность, время)},label=lst:qscore-go]
// sm2QScore выводит q in [0,5] из факта правильности и скорости ответа.
//   correct + <=halfT -> 5;  correct + <=T -> 4;  correct + >T -> 3;
...
type XPParams struct {
    Beta      float64 // штраф за медлительность; 0.5 по умолчанию
...
% --- Приложение Б ----------------------------------------------------------
\newpage
\section*{ПРИЛОЖЕНИЕ Б\\[2pt]\normalfont\normalsize\mdseries Скриншоты интерфейса}
\label{application_B}
\addcontentsline{toc}{section}{ПРИЛОЖЕНИЕ Б. Скриншоты интерфейса}

% Перенумерация для приложения Б
...
функциональность, описанную в главе~3 диссертации. Скриншоты получены
в среде Flutter Web с использованием демонстрационных учётных
записей.
...
\image{ui-profile.png}{Профиль обучающегося: очки опыта, уровень, достижения}{fig:ui-profile}{0.7}
...
% --- Приложение В ----------------------------------------------------------
\newpage
\section*{ПРИЛОЖЕНИЕ В\\[2pt]\normalfont\normalsize\mdseries Протоколы педагогического эксперимента}
\label{application_V}
...
эксперимента в соответствии с методикой, описанной
в подразделе~3.7 диссертации. Заполненные протоколы прилагаются
к работе после завершения апробации в текущем семестре.

\subsection*{В.1\quad Форма анкеты участника эксперимента}

Анкета заполняется обучающимся перед началом эксперимента
...
курса \textit{«Базы данных»} (терминология реляционной модели,
нормальные формы, язык SQL, элементы транзакционной обработки).
Каждый тест проводится в одинаковых условиях и в течение
одинакового времени.
\begin{itemize}
...
для программного пайплайна статистического анализа (подраздел~3.7).
...
% --- Приложение Г ----------------------------------------------------------
\newpage
\section*{ПРИЛОЖЕНИЕ Г\\[2pt]\normalfont\normalsize\mdseries Список сокращений и обозначений}
\label{application_G}
