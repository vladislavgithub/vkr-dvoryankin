...
\usepackage{caption}
% Методичка кафедры АСУ предписывает длинное тире (—) после номера
% в подписях: 'Рисунок 1 — Описание', 'Таблица 1 — Заголовок'.
% Встроенного labelsep=emdash в caption нет; объявляем собственный.
...
\captionsetup[table]    {name=Таблица, labelsep=emdash, justification=raggedleft, singlelinecheck=off, position=above}
\captionsetup[figure]   {name=Рисунок, labelsep=emdash, justification=centering,  singlelinecheck=on,  position=below}
\captionsetup[lstlisting]{labelsep=emdash, justification=centering, singlelinecheck=on,  position=below}
...
% Короткое тире en dash в диапазонах страниц '23–31'.
% biblatex-gost подменяет \bibrangedash на em dash для русского
% режима; перекрываем через \renewcommand и \AtBeginBibliography
% (более позднее срабатывание, чем \AtBeginDocument).
\renewcommand*{\bibrangedash}{\textendash}
\AtBeginBibliography{\renewcommand*{\bibrangedash}{\textendash}}
